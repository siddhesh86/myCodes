...
#sDateTime = "tmp" 
fAnaEvtYieldLog = open("analyzeEvtYield_HHTo4W_3l_1qq_inDetail_%s.log" % sDateTime, 'w')
...
def analyzeEvtYield(histogramWorkingDir):    
    ...
    sHistoAdded  = "%s/hist_stage1_%s_inDetail.root" % (histogramWorkingDir,ElectronID)
    ...
    # event yield processwise
    hStat_Catwise       = OD()
    hStat_Added_Catwise = OD()
    nEvts_Catwise       = OD()
    nEvts_Added_Catwise = OD()
    ...
    for sample_name, sample_info in samples.items():
        ...
        sfHist =  "%s/%s/hist_stage1_%s_%s.root" % (histogramWorkingDir, sample_info["process_name_specific"], sample_info["process_name_specific"], ElectronID_part1)
        fHist = ROOT.TFile(sfHist)
        if not fHist.IsOpen():
            print "Couldn't open %s \t\t ***** ERROR ***** " % sfHist
        
        hStat     = fHist.Get("%s/Stat1" % sample_info["sample_category"])
        hStat_wtd = fHist.Get("%s/Stat1_wtd" % sample_info["sample_category"])
        hStat.SetDirectory(0)
        hStat_wtd.SetDirectory(0)
        
        nEvts         = hStat_wtd.GetBinContent(kBin_selection_cut_Last)
        errnEvts      = hStat_wtd.GetBinError(kBin_selection_cut_Last)
        nEvts_UnWtd   = hStat.GetBinContent(kBin_selection_cut_Last)
        print "Category %s: %g  +-  %g (%g) \t %s %g %s" % (sample_info["sample_category"], nEvts,errnEvts,nEvts_UnWtd, selection_cut_Last,kBin_selection_cut_Last,sBinLabel_tmp)
        hStat_Processwise = OD()
        hStat_Processwise[sample_info["process_name_specific"]] = OD([ ("hStat_wtd", hStat_wtd),  ("hStat", hStat) ])
        
        nEvts_Processwise = OD()
        nEvts_Processwise[sample_info["process_name_specific"]] = OD([ ("nEvents",nEvts), ("errNEvents",errnEvts), ("nEvents_unweighted",nEvts_UnWtd) ])
        
        if sample_info["sample_category"] not in hStat_Catwise:
            hStat_Catwise[sample_info["sample_category"]] = hStat_Processwise
            nEvts_Catwise[sample_info["sample_category"]] = nEvts_Processwise
            hStat_Added_Catwise[sample_info["sample_category"]] = OD([ ("hStat_wtd_added", hStat_wtd),  ("hStat_added", hStat) ])
            nEvts_Added_Catwise[sample_info["sample_category"]] = OD([ ("nEvents",nEvts), ("errNEvents",errnEvts), ("nEvents_unweighted",nEvts_UnWtd) ])
        else:
            hStat_Catwise[sample_info["sample_category"]].update( hStat_Processwise )
            nEvts_Catwise[sample_info["sample_category"]].update( nEvts_Processwise )
            
            hStat_Added_Catwise[sample_info["sample_category"]]["hStat_wtd_added"].Add(hStat_wtd)
            hStat_Added_Catwise[sample_info["sample_category"]]["hStat_added"].Add(hStat)
            
            nEvts_Added_Catwise[sample_info["sample_category"]]["nEvents"]            = hStat_Added_Catwise[sample_info["sample_category"]]["hStat_wtd_added"].GetBinContent(kBin_selection_cut_Last)
            nEvts_Added_Catwise[sample_info["sample_category"]]["errNEvents"]         = hStat_Added_Catwise[sample_info["sample_category"]]["hStat_wtd_added"].GetBinError(kBin_selection_cut_Last)
            nEvts_Added_Catwise[sample_info["sample_category"]]["nEvents_unweighted"] = hStat_Added_Catwise[sample_info["sample_category"]]["hStat_added"].GetBinContent(kBin_selection_cut_Last)
            



    print "nEvts_Catwise:: ",json.dumps(nEvts_Catwise, sort_keys = False, indent = 4, ensure_ascii = False)
    #print "hStat_Catwise:: ",json.dumps(hStat_Catwise, sort_keys = False, indent = 4, ensure_ascii = False)
    print "nEvts_Added_Catwise:: ",json.dumps(nEvts_Added_Catwise, sort_keys = False, indent = 4, ensure_ascii = False)
    ...
    fAnaEvtYield_EleID = open("%s/EventYield_%s_inDetail.txt"  % (histogramWorkingDir, ElectronID), 'w')
    ...
    sline = "%-60s \t %35s \t %35s \t %35s \n\n" % ("Sample ", "No. of selected events", "Error on no. of selected events", "No. of selected unweighted events")
    fAnaEvtYield_EleID.write(sline);    fAnaEvtYieldAll.write(sline);
    for sample_category, evtYieldDetails in evtYield_S_dict.items():
        ...
        sline = "%-60s \t %25.3f \t %25.3f \t %25d \n" % (sample_category, nEvts_Added_Catwise[sample_category]["nEvents"], nEvts_Added_Catwise[sample_category]["errNEvents"], nEvts_Added_Catwise[sample_category]["nEvents_unweighted"])
        fAnaEvtYield_EleID.write(sline);    fAnaEvtYieldAll.write(sline);
    sline = "\n"
    fAnaEvtYield_EleID.write(sline);    fAnaEvtYieldAll.write(sline);
    for sample_category, evtYieldDetails in evtYield_B_dict_sorted.items():
        ...
        sline = "\n%-60s \t %25.3f \t %25.3f \t %25d \n" % (sample_category, nEvts_Added_Catwise[sample_category]["nEvents"], nEvts_Added_Catwise[sample_category]["errNEvents"], nEvts_Added_Catwise[sample_category]["nEvents_unweighted"])
        fAnaEvtYield_EleID.write(sline);    fAnaEvtYieldAll.write(sline);
        for process, evtYieldOfProcess in nEvts_Catwise[sample_category].items():
            sline = "%-60s \t %25.3f \t %25.3f \t %25d \n" % ("    "+process, evtYieldOfProcess["nEvents"], evtYieldOfProcess["errNEvents"], evtYieldOfProcess["nEvents_unweighted"])
            fAnaEvtYield_EleID.write(sline);    fAnaEvtYieldAll.write(sline);
...
if __name__ == "__main__":
    ...
    fAnaEvtYieldRootFile = ROOT.TFile("%s/analyzeRun_hh_3l%s/%d/analyzeEvtYield_HHTo4W_3l_1qq_inDetail.root" % (pwd, anaVersion, era), "RECREATE")
    fAnaEvtYieldAll = open("%s/analyzeRun_hh_3l%s/%d/EventYield_ElectronID_All_inDetail.txt"  % (pwd, anaVersion, era), 'w')
